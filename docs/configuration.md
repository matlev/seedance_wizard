...
**Media split behavior** controls which resulting clip owns the frame selected at the playhead. The default **Split before selected frame** makes that frame the first frame of the second clip. **Split after selected frame** makes it the last frame of the first clip and begins the second clip at the next decoded frame. The choice applies to subsequent splits after Settings closes; existing clips and immutable recipe revisions do not change.
...
The current materializer executes this flow for physical assets, pinned virtual-asset revisions, and exact frame anchors on physical or pinned virtual videos. Hosting and provider preparation receive transient realized media without changing the logical provenance recorded in the project. Virtual-video positions remain anchored to the exact source recipe revision and realized content identity that defined them.
