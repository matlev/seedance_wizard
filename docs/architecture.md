...
An existing audio clip can be dragged horizontally to revise that offset. Pointer movement previews placement from the current timeline projection without changing project state; mouse-up commits the final tick-precise time as one immutable recipe revision. A click, cancelled drag, out-of-bounds release, or release at the existing time is a no-op. Video reorder and audio placement therefore share the same draft-during-gesture and commit-on-release rule.
