...
Settings are a focused WPF feature under `Views/Settings`; reusable modal interactions live under `Views/Dialogs`. The settings window owns section navigation, validation, commit/close behavior, credential actions, and diagnostic-log relocation. A dedicated settings-field factory maps configuration catalog metadata to dynamic WPF controls without owning persistence. After the modal window closes, `ApplicationRuntime` reloads settings and reapplies media-tool executables, cache size, retention preference, and cache trimming to the runtime services it owns; `MainWindow` refreshes only provider/UI state that genuinely belongs to the active shell.
