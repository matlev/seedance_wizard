...
Phase 2E begins by adding a visual projection over that same committed recipe. Duration-aware segment spans, readable minimum display widths, ruler ticks, selection highlighting, zoom scale, scrolling, and the playback playhead are derived UI state and never enter project persistence. The projection may estimate missing source duration for layout while labeling the result as estimated; render planning continues to inspect authoritative realized media rather than trusting that estimate. Timeline zoom uniformly scales blocks and ruler geometry while preserving composition-time mapping for the playhead, insertion, reorder, Project Media drops, and audio placement. Video and audio identity badges follow the visible left edge of their block while remaining clipped inside that block; near its trailing edge they compress to the remaining space, with the complete identity and mix details retained in the tooltip. When a current composition revision has a rendered preview, the ruler is also a navigation surface: click or drag maps its projected position back to preview time without changing recipe state, and a paused preview remains paused after the gesture. A default-on, immediately toggleable auto-scroll follows the playhead only after it leaves the visible viewport, placing it near the left edge when possible and clamping at the final scroll extent. That toggle is intentionally session-only UI state rather than a persisted project or application preference. A preview is associated with the exact composition revision it rendered, so committing a new revision invalidates the old preview/playhead relationship.
