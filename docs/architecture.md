...
The Generate surface under `Views/Generation` owns provider capability presentation, prompt and generation-setting controls, reference-grid editing and duplication, draft form capture/loading, and typed intents for provider changes, lineage actions, reference preview, and submission. Presentation reference choices and prompt preview formatting live with that feature. The control never invokes a provider or authorizes a billable action. The shell translates its typed intent into the application generation workflow, where explicit human cost authorization, immutable snapshot construction, reference preparation, Undo Send, submission, monitoring, and finalization remain enforced.
