...
- a selected audio clip exposes timing plus implemented On/Muted, gain, fade-in, and fade-out controls;
...
Independent audio clips persist `IsMuted`, a finite `GainDecibels` value from -60 dB through +12 dB, and non-negative millisecond-normalized fade-in/fade-out durations. All mix properties participate in recipe validation, render-plan/cache identity, persistence, and FFmpeg filter construction before per-clip delay and final mixing. Fade-in begins at the audio source's audible start. Fade-out targets the end of the portion audible within the composition, so it still completes when a longer audio source is cut short by the video. The mixed stream is padded and the output remains bounded by video duration, so a short audio clip cannot truncate the composition and a long one cannot extend it. Radio changes commit discretely; pointer movement on gain or fade sliders changes only UI draft state until release, while a completed keyboard adjustment commits its final value.
