...
The media viewer visual tree and its low-level WPF event source live under `Views/MediaPreview` in `MediaPreviewPanel`. The panel owns video, still-image, placeholder, transport-chrome, and independent composition-audio elements while exposing semantic presentation operations and interaction events. During the staged extraction, `MainWindow` temporarily retains transport and composition policy through an explicit compatibility surface; subsequent slices move that state machine behind the media-preview boundary without combining a visual-tree relocation with playback-policy changes.
