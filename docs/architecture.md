...
The approved [Gate 0 media capability charter](gate-0-media-capability-charter.md) separates ReelForge's semantic required capabilities, concrete observed-runtime capabilities, and runtime-profile mappings. The Checkpoint-A-approved minimum observation/validation seam is implemented for Gate 0 tests and CI reporting: Application owns engine-neutral immutable observation/assessment contracts, Infrastructure performs explicit paired-runtime identity/component/parser probes and strict reviewed-manifest comparison, and generated proof evidence remains operational rather than project truth. Component presence is separate from executed semantic proof. Core and project persistence contain no engine names, executable paths, FFmpeg filter names, or Windows-only acceleration concepts. The seam does not change production discovery, renderers, caches, Settings, or user-facing behavior; broader production adoption still requires separate approval and an ADR.

The G0.3 text proof preserves the same boundary. Script/language-run-to-font mapping, exact font-artifact selection, DirectWrite-provider controls, ASS documents, and FFmpeg filter choices are Infrastructure/runtime-profile evidence. They are not Core concepts, persisted project meaning, final UI parameters, or a Windows dependency that portable composition semantics may assume. Any production text-rendering adoption must expose engine-neutral title/caption intent through Application ports, keep platform/provider selection behind Infrastructure or platform adapters, and receive a separate ADR before changing the production architecture.
