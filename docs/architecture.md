...
The approved [Gate 0 media capability charter](gate-0-media-capability-charter.md) targets a separation between ReelForge's semantic `RequiredCapabilities`, concrete `ObservedRuntimeCapabilities`, and runtime-profile mappings. The proposed boundary keeps semantic requirements and preflight policy in Application, concrete FFmpeg/runtime observation and mapping in Infrastructure, and any later availability/failure presentation in App. Core and project persistence retain creative intent and typed settings without engine names, executable paths, FFmpeg filter names, or Windows-only acceleration concepts. Gate 0 has not implemented this boundary: Checkpoint A must approve the exact scope of any smallest reusable discovery/validation component, and a durable resulting contract receives an ADR. Editing behavior and broader engine frameworks require later approval.
