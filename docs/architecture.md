...
The final [media runtime contract](media-runtime-contract.md) separates ReelForge's semantic capabilities, observed runtime capabilities, and concrete profile mappings. Application owns engine-neutral immutable observation/assessment contracts; Infrastructure performs explicit paired-runtime identity/component/parser probes and strict reviewed-profile comparison. Component presence is not proof that a product workflow works. Core and project persistence contain no engine names, executable paths, FFmpeg filter names, or Windows-only acceleration concepts. Production runtime discovery and diagnostics may extend this seam without changing portable project meaning.

Text rendering preserves the same boundary. Script/language-run-to-font mapping, exact font selection, ASS documents, and FFmpeg filter choices are Infrastructure/runtime-profile concerns. They are not Core concepts, persisted project meaning, final UI parameters, or a Windows dependency that portable composition semantics may assume. Production text rendering exposes engine-neutral title/caption intent through Application ports and keeps platform/provider selection behind Infrastructure or platform adapters.

Free desktop 1.0 workspace ergonomics will allow the composition timeline and Edit Tools surfaces to detach into independent floating windows and dock back into the main Edit workspace. Docked and floating presentations remain views over one authoritative composition, selection, undo/history, playback, and project state rather than opening duplicate editor sessions. Window size, position, monitor, and dock state are machine-local UI preferences, ideally remembered per project, and recover safely into visible bounds when a previously used monitor is unavailable. This is App/platform presentation state, not project-schema state; WPF window and monitor concepts must not enter portable Core or Application contracts.
...
Settings are a focused WPF feature under `Views/Settings`; reusable modal interactions live under `Views/Dialogs`. The settings window owns section navigation, validation, commit/close behavior, credential actions, and diagnostic-log relocation. A dedicated settings-field factory maps configuration catalog metadata to dynamic WPF controls without owning persistence. The **Project** section is deliberately outside that application-settings catalog: it presents commands against the currently active workspace, and `ProjectLocation` remains the sole authority for the project folder. Selecting another location or running cleanup therefore invokes an explicit project operation and never writes a parallel location or cleanup preference into application settings or the `.rfp`. After the modal window closes, `ApplicationRuntime` reloads application settings and reapplies media-tool executables, cache size, retention preference, and cache trimming to the runtime services it owns; `MainWindow` refreshes only provider/project/UI state that genuinely belongs to the active shell.

Project creation, project opening, transfer-target selection, and media-import file picking use a focused WPF dialog policy under `Views/Projects`. It returns user selections but cannot mutate a workspace. The Project Media control under `Views/ProjectMedia` owns grouped rendering, context-action intent, right-click selection, and composition-drag initiation. Its Videos, Audio, Saved Frames, Saved Clips, and Compositions headings will be independently collapsible and expandable so large projects can hide irrelevant groups; this is presentation state and does not alter project media or domain persistence. A future multi-select mode will support bulk delete, copy, and move workflows without weakening the existing per-item dependency, physical-versus-virtual, and copy-only eligibility rules. Bulk destructive work must summarize affected items and obtain one clear confirmation without silently applying a surprising partial deletion; long-running transfers require progress, cancellation, and per-item results. Project mutations, preview/Inspector reactions, generation-reference synchronization, and composition consequences remain outside that view so they can move behind their respective feature boundaries rather than turning the media list into a second shell. The focused Inspector view owns read-only presentation and formatting for media identity/encoding, Saved Frame provenance, generation history, and detailed failures; it never mutates the selected object or initiates provider work.

Project cloning is an Application-owned use case over the existing project-store format boundary and an Infrastructure-owned staged filesystem transaction. It reads only the selected project's last committed `.rfp`, gives the clone a new project identity and requested name, and preserves every internal asset, recipe, anchor, generation, provenance, and composition identity. Infrastructure copies project-local durable content without interpreting it, excluding disposable `cache/`, recovery candidates, and interrupted-write artifacts. The copy is built in a uniquely owned sibling staging directory, saved and reopened through `IProjectStore`, then published by a non-overwriting directory rename. Cancellation or failure removes only that staging directory; an abnormal process exit cannot expose a partially published clone. Reserved unpublished-staging names are ignored by project discovery, direct project selection, and later clone scans. WPF owns source/destination selection and the cancellable progress window. Cloning never activates recovery, changes the current workspace, reconciles project media, runs Project Cleanup, cleans cache, or reaches provider authorization.

Project relocation is a distinct Application-owned lifecycle operation over one focused Infrastructure filesystem transaction. It preserves the project identity and complete folder contents, including project-local cache, while changing only the machine-local `ProjectLocation`. Clean, saved, and degraded sessions may move; dirty, saving, recovered, recovery-available, and failed sessions must first resolve their lifecycle state. Same-volume moves use a directory rename. Cross-volume moves copy into a uniquely owned hidden destination-sibling staging folder, reopen the staged `.rfp` through `IProjectStore`, publish without overwrite, rebind the exact active workspace from the already validated project, and then retire the old folder. No fallible project-store reopen occurs after publication. After a successful relocation, the shell updates recent/last-project paths. Cancellation before publication leaves the source authoritative; after publication the operation completes its rebind rather than treating caller cancellation as rollback. Active or unreconciled generation jobs bound to the old path block relocation so job finalization cannot target a stale location.

Physical Project Media deletion is always available to the user. An unreferenced physical asset is removed outright; a referenced physical asset becomes a persisted deleted tombstone and is hidden from ordinary Project Media and new-reference choices while its stable identity and historical metadata remain resolvable. In both cases the contained media file is removed only after project state commits successfully. Dependents may consequently become degraded, but deletion does not cascade, rewrite history, reconcile the `.rfp`, prune cache entries, or initiate provider work. [ADR-0002](adr/0002-referenced-project-media-deletion.md) records this persistence rule.

Hash-equivalent bytes can restore a deleted tombstone only through an explicit user-authorized restoration decision. The verified recovery path reuses the original `AssetId` and project-relative storage rather than rewriting dependents or treating equal content as general deduplication. Ambiguous matches require selection, and import-as-new remains available. [ADR-0003](adr/0003-verified-deleted-source-restoration.md) records the restoration rule.

**Cleanup Project** is the explicit cascade that physical deletion deliberately omits. Before the first Project Media projection after a stable open, and again immediately before cleanup, the existing Infrastructure availability seam reconciles all active physical sources in one recovery-aware save; matching persisted size/time signatures avoid redundant SHA-256 work, while changed verified files are hashed. Application dependency analysis then marks active Saved Frames, Saved Clips, and compositions degraded when their exact pinned dependency graph reaches a deleted, missing, inaccessible, or mismatched physical source. Cleanup archives the affected active anchors, tombstones the affected virtual assets, and clears a degraded Working Composition selection in one recovery-aware project save. Recipes, revisions, provenance, generation snapshots, and logical identities remain hidden historical evidence rather than being rewritten into dangling references. Temporarily missing media is included, so confirmation warns that cleanup is irreversible even though relinking could otherwise restore it. Healthy durable extracted audio is not degraded merely because provenance names an unavailable source. Unsafe lifecycle states and active or unreconciled generation jobs block cleanup. Cleanup does not delete or reconcile cache files, scan for unrelated filesystem orphans, render media, export media, or reach a provider. [ADR-0004](adr/0004-explicit-derived-media-cleanup.md) records the retained-history rule.

Cache availability shown for degraded Project Media is advisory machine state. Infrastructure records only successfully produced derived representations in a disposable cache-local index keyed by project and exact asset/anchor revision identity. The index and every referenced cache file may disappear at any time and never enter project persistence. A degraded item may offer ordinary export as a rescue path when an indexed representation still validates; this reads existing bytes without rendering, changing the project, or weakening the cleanup dependency analysis.

The Generate surface under `Views/Generation` owns provider capability presentation, prompt and generation-setting controls, reference-grid editing and duplication, draft form capture/loading, and typed intents for provider changes, lineage actions, reference preview, and submission. Presentation reference choices, prompt preview formatting, and the full-workspace expanded prompt editor live with that feature. The expanded editor owns bidirectional text synchronization, focus, close, and Escape-key behavior while the shell only places its overlay and reconnects normal prompt focus. Its sibling Generation History view owns history projection, concise prompt/status rendering, selection retention, and selected-generation lookup. None of these controls invokes a provider or authorizes a billable action. The shell translates typed intent into the application generation workflow, where explicit human cost authorization, immutable snapshot construction, reference preparation, Undo Send, submission, monitoring, and finalization remain enforced. The Edit Tools surface under `Views/Editing` similarly owns context-sensitive segment/audio presentation, input suppression, value formatting, and typed property-change intents. It does not mutate composition recipes; the shell/application boundary applies each requested immutable recipe change and refreshes selection state.
...
- unexplained dangling source references that are not backed by an explicit deleted-asset tombstone;
- timeline clips whose logical source cannot produce the requested media type.

Deleting a virtual asset should not delete its physical sources. Deleting a physical source with dependents requires an explicit dependency-aware decision and retained logical tombstone; it never silently deletes or rewrites dependents. Cache files never participate in referential integrity.
...
The Application editing boundary keeps `WorkingCompositionService` as a source-compatible facade while assigning current-composition access, lifecycle creation, segment commands, audio commands, and exact split mutation to focused internal owners. `TransactionalCompositionRevisionEditor` is the single ordinary-edit owner for cloning a recipe, committing its immutable revision, refreshing provenance and the editable draft, saving, and restoring all touched state after failure. Split and detached-audio workflows retain their specialized media preparation but delegate the project mutation directly to the corresponding focused owner; their outer rollback also removes newly created anchors, assets, revisions, and files and restores project timestamps.

Phase 2E begins by adding a visual projection over that same committed recipe. Duration-aware segment spans, readable minimum display widths, ruler ticks, selection highlighting, zoom scale, scrolling, and the playback playhead are derived UI state and never enter project persistence. The projection may estimate missing source duration for layout while labeling the result as estimated; render planning continues to inspect authoritative realized media rather than trusting that estimate. Timeline zoom uniformly scales blocks and ruler geometry while preserving composition-time mapping for the playhead, insertion, reorder, Project Media drops, and audio placement. Video and audio identity badges follow the visible left edge of their block while remaining clipped inside that block; near its trailing edge they compress to the remaining space, with the complete identity and mix details retained in the tooltip. The ruler is a navigation surface for both rendered previews and fast composition audition: click or drag maps its projected position to composition time without changing recipe state, and a paused preview remains paused after the gesture. Fast audition lazily realizes one pinned video source at a time, respects each segment's committed source-audio inclusion, and switches sources at cuts, avoiding a complete composition bake during ordinary arrangement and split work. Independent audio clips are rendered into a cached audio-only audition stream with their committed timeline offset, mute, gain, pan, and fades, then synchronized to the source-by-source video player across play, pause, seek, frame-step, and segment changes. Separate playback pipelines can still hiccup or differ slightly from final FFmpeg normalization at seams, so **Preview composition** remains the authoritative check for mix fidelity and final render continuity. Project Media playback never drives the composition playhead; clicking the timeline reselects the Working Composition before timeline navigation. A default-on, immediately toggleable auto-scroll follows the playhead only after it leaves the visible viewport, placing it near the left edge when possible and clamping at the final scroll extent. That toggle is intentionally session-only UI state rather than a persisted project or application preference. A rendered preview remains associated with the exact composition revision it rendered, so committing a new revision invalidates that rendered artifact while the editor can immediately rebuild its lightweight source-by-source audition state. Within the current application session, ReelForge also remembers the exact project, composition, and recipe revision of the last explicit rendered preview: switching Project Media releases the active viewer lease but reselecting the unchanged composition reacquires the deterministic cached representation. This preference is never project state, is discarded on project changes, and cannot survive a recipe-revision mismatch; persistence across application restart remains a separate cache-discovery concern.

`Views/Editing/CompositionTimelineControl` owns timeline rendering, ruler/zoom/playhead, session auto-scroll, local selection and gestures, context/hover presentation, and Project Media drop feedback with edge-scroll timers. It receives one atomic App-local projection state and emits semantic requests; project mutation, split/detach policy, Project Media activation, Edit Tools, and audition/MediaPreview orchestration remain at shell or application boundaries. Playback updates are non-rendering, while structural state synchronously installs its timing projection so shell split and fade queries are current.

Fast-audition timeline construction and position mapping belong to the portable `CompositionAuditionPlan`. It validates pinned segment sources and durations, resolves exact source boundaries, projects contiguous composition spans, selects the trailing segment at a cut, and maps source time to composition time in both directions. `CompositionAuditionSession` owns the active segment and global/source position transitions for one pinned recipe revision. The WPF `CompositionAuditionController` consumes that session and owns materialized segment lease sequencing, stale asynchronous-open rejection, cut advancement, seeking, replay, independent-audio synchronization, and coalesced cross-cut scrub requests. The shell observes semantic position changes for timeline projection rather than duplicating audition state.
...
`CompositionRenderCoordinator` owns the Edit workspace's one-at-a-time preview/export state machine: exact project/location/composition/revision capture, interaction suppression, audition quiescence, cancellation, stale project/revision/selection rejection, materialization lease transfer, error/status policy, and final UI cleanup. Its host keeps WPF save dialogs, viewer publication, and control visibility at the shell boundary. A project reset invalidates and cancels active render work before preview state is cleared, and a changed revision or Project Media selection cannot receive a late baked preview. `MediaPreviewCoordinator` remains the sole owner of viewer, audition, and retained baked-preview state.
...
This whole-source action remains distinct from timeline **Detach audio**. Detach materializes the selected segment's exact revision-pinned in/out range, extracts a durable M4A/AAC asset, and adds that asset as an independent audio clip at the segment's exact composition start. The same composition revision mutes only the selected segment's embedded source audio to prevent doubling; pre-existing audio clips remain in place and overlap/mix normally. Provenance pins the composition revision and segment that produced the file. This is intentionally a permanent derived asset because it cannot be reconstructed solely from the later composition after the source segment or its boundaries change.
...
The WPF Jobs surface is composed from two focused controls. The global pane owns tracked-job projection, elapsed and Undo Send countdown updates, terminal acknowledgement, dismissal after leaving the pane, and cancellation/error events. The top-chrome control owns status-change indication and the active-job mascot animation. ReelForge's custom WPF title bar hosts that control alongside the application menu, workspace selectors, project label, and native window commands; it only routes foreground presentation intent to the existing coordinators and does not become a second application-command path. Neither Jobs control submits or monitors provider work; both observe the application-scoped coordinator. The shell bridges a local cancellation request only to the matching in-memory Undo Send delay, keeping potentially billable submission authorization outside passive Jobs UI behavior.
...
The current single-document JSON representation is not assumed to be the permanent storage engine. Immutable recipe, anchor, and generation history can grow much faster than media duration alone suggests, while the current implementation reads and validates the complete document on open and validates and atomically rewrites it on save. Before the first supported project-format baseline, benchmark representative projects by serialized size and historical-record count, including open/save latency, peak memory, and write amplification. Use measured thresholds—initially 200 ms for an ordinary save or one second for open—as the trigger for a storage redesign. Possible outcomes include safe history compaction, an authoritative sidecar/history store, or an indexed transactional database; none is selected in advance. Any replacement must preserve immutable referenced revisions, corruption-safe commits, logical project identity, and cache independence. Changing only the `IProjectStore` implementation is not sufficient if aggregate loading and validation still require all history in memory, so the investigation must cover both persistence and domain access patterns.
