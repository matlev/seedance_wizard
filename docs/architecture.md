...
Fast-audition timeline construction and position mapping belong to the portable `CompositionAuditionPlan`. It validates pinned segment sources and durations, resolves exact source boundaries, projects contiguous composition spans, selects the trailing segment at a cut, and maps source time to composition time in both directions. WPF remains responsible for opening materialized segment leases and presenting playback, but it must consume this plan rather than duplicate boundary and cut arithmetic in the shell.
