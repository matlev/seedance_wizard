...
The Generate surface under `Views/Generation` owns provider capability presentation, prompt and generation-setting controls, reference-grid editing and duplication, draft form capture/loading, and typed intents for provider changes, lineage actions, reference preview, and submission. Presentation reference choices and prompt preview formatting live with that feature. Its sibling Generation History view owns history projection, concise prompt/status rendering, selection retention, and selected-generation lookup. Neither control invokes a provider or authorizes a billable action. The shell translates typed intent into the application generation workflow, where explicit human cost authorization, immutable snapshot construction, reference preparation, Undo Send, submission, monitoring, and finalization remain enforced. The Edit Tools surface under `Views/Editing` similarly owns context-sensitive segment/audio presentation, input suppression, value formatting, and typed property-change intents. It does not mutate composition recipes; the shell/application boundary applies each requested immutable recipe change and refreshes selection state.
