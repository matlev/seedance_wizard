...
Status: in progress. The dedicated WPF App test project now provides focused coordinator coverage, and deterministic App and Platform.Windows assembly-reference checks enforce their direct production-layer dependencies. The first portable split is complete: Application-only tests now live in `ReelForge.Application.Tests`, Infrastructure-focused tests now live in `ReelForge.Infrastructure.Tests`, and the remaining broad `ReelForge.Tests` suite retains cross-layer coverage. GitHub Actions restores and runs all three portable suites on Ubuntu; Windows builds Debug and Release, then runs all five suites in Release. The broader layer-by-layer split of the remaining portable suite remains staged work.
