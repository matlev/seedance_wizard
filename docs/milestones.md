...
The media viewer visual tree and low-level WPF event source now live in `Views/MediaPreview/MediaPreviewPanel`. The shell temporarily retains transport and composition policy through an explicit compatibility surface while the playback state machine moves behind that boundary in subsequent Stage 6 slices.
