...
Progress: the plan is approved. Stage 0 established executable dependency-direction checks, a manual acceptance matrix, and concurrent atomic active-job persistence. Stage 1 introduced the Windows platform/test projects, moved machine-local path and Credential Manager ownership behind portable contracts, injected explicit settings/job/log/cache/project locations, neutralized portable secure-store wording, and moved service construction, provider HTTP-client ownership, job-coordinator construction, and disposal into `Bootstrap/ApplicationRuntime`. Stage 2 is complete: Core models are organized by domain family, project invariants delegate through aggregate-specific validators, Application configuration/contracts have capability-owned homes, and the unused duplicate `ProjectWorkspace.SubmitGenerationAsync` path was removed after migrating its characterization to `GenerationWorkflow`. Stage 3 is in progress: the project persistence DTO and mapper monolith has been separated by project-root, asset, recipe, anchor, and generation aggregate families behind the unchanged atomic store facade; one canonical project-path policy rejects rooted and parent-relative escapes using host-appropriate path semantics; media workflows share filename validation plus collision-free destination policies without changing their visible naming conventions; and project/settings/job writes plus media import, ingestion, extraction, promotion, and export share a same-directory atomic commit scope with automatic partial-file cleanup. The complete suite currently consists of 213 portable tests and 3 Windows-platform tests.
