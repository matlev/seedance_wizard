...
The media viewer visual tree and transport state machine now live in `Views/MediaPreview/MediaPreviewPanel`. The focused surface owns player priming, play/pause/replay state, scrubbing capture, position/volume presentation, mute policy, silent precision mode, frame-step intent, and independent audition-audio synchronization without exposing its WPF controls. The shell retains composition sequencing, exact-frame lookup, materialized-lease ownership, timeline playhead projection, and failure routing as cross-feature policy.
