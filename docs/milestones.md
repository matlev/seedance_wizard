...
The next Stage 6 boundary is also established: Settings and reusable dialogs have explicit `Views` homes, dynamic settings-control construction is separated from persistence/window lifecycle, and the composition root reapplies changed media-tool, cache, and retention settings to runtime-owned services.
