...
| BYOK generation | Implemented | Fake, BytePlus Seedance 2.5, AtlasCloud Seedance 2.5, and AtlasCloud MiniMax H3 routes; capability-driven forms; references; explicit Atlas source-video edit generation; immutable snapshots; history; lineage; retry/variant/branch/continuation; async jobs; output ingestion; and human-only billable authorization exist. Edit generation creates new durable Project Media and is distinct from future provider-assisted repair or direct replacement. Live provider availability remains external. |
