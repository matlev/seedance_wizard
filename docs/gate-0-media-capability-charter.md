...
The bounded execution sequence and evidence semantics are defined in the [G0.5 calibration plan](gate-0-g0.5-calibration-plan.md). Its immutable [Stage 1 results](gate-0-g0.5-calibration-results.md) retain 24 complete VP9/Opus passes and 24 OpenH264/AAC failures under the superseded maximum-delta audio check. The owner approved and froze the corrected oracle described by the [audio-oracle proposal](gate-0-g0.5-lossy-audio-oracle-proposal.md). Its [retained-output follow-up](gate-0-g0.5-retained-audio-results.md) evaluated all 48 artifacts without re-encoding and passed all 48. The bounded [marker-survivability qualifier](gate-0-g0.5-marker-survivability-results.md) then passed both exact route profiles across 1,500 frames with zero identity, collision, ambiguity, count, or PTS deviations. The isolated adapter and its [visible no-media control](gate-0-g0.5-wpf-no-media-results.md) passed the normal-host reference with zero media children and complete cadence/host evidence. The owner-approved [versioned Stage 2 workload contract](gate-0-g0.5-stage2-workload-proposal.md) and [proof-only P2 WPF boundary](gate-0-g0.5-wpf-measurement-adapter-boundary.md) remain in force. Complete remote-copy verification and resource/free-space preflight remain before bounded smoke; media-load, human-observation, preview, cancellation, and long-form evidence remain unexecuted. The full Stage 2 matrix is not yet authorized.
...
The [G0.4 executable delivery-proof results](gate-0-g0.4-executable-proof.md) record eleven passed portable output routes and two passed optional W1 variants. The [independent-playback checkpoint](gate-0-independent-playback-checkpoint.md) adds two native-Chromium WebM control passes, explicit MP4 long-corpus blocks, and optional WMP Legacy WebM blocks without finalizing the default or shipping route. The corrected [common-input result](gate-0-g0.4-input-proof-results.md) plus the retained [P3 JPEG follow-up](gate-0-g0.4-p3-jpeg-results.md) now retain 173 passes and 83 failures. The F7 correction is blocked, so the six-family direct-Matroska pilot remains blocked. G0.5 Stage 1, the frozen [retained-audio follow-up](gate-0-g0.5-retained-audio-results.md), and bounded [marker-survivability qualification](gate-0-g0.5-marker-survivability-results.md) are complete for both route profiles. The dedicated private R2 target and [durable artifact-retention contract](gate-0-artifact-retention.md) now replace the unspecified second-copy location, but complete migration and independent remote byte verification remain incomplete. Remaining playback rows, the WPF and resource prerequisites, G0.5 Stage 2 smoke/quality/performance/long-form evidence, and release-engineering/legal work stay open. OpenH264's no-frame-skip bitrate warning remains an explicit G0.5 quality-policy finding.
