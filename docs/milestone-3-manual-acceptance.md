...
| Audition | Audition an unbaked composition, drag the player and timeline scrubbers rapidly across at least three cuts, release and resume playback, then seek/frame-step and play across muted segments plus independent layered audio. | Transient cross-cut seeks do not strand the player; pointer-up selects the final requested position and preserves the prior play/pause state. Playback follows the current recipe without advancing during unrelated Project Media playback; mute/gain/pan/fade and independent audio are respected. |
