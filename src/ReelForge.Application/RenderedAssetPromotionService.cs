...
public sealed class RenderedAssetPromotionService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (!Path.GetExtension(fullDestinationPath).Equals(".mp4", StringComparison.OrdinalIgnoreCase))
            throw new ArgumentException("Composition exports must use the .mp4 file type.", nameof(destinationPath));
    ...
    private static string ValidateMp4FileName(string value)
    {
        ...
        if (!Path.GetExtension(fileName).Equals(".mp4", StringComparison.OrdinalIgnoreCase))
            throw new ArgumentException("Rendered composition assets must keep the .mp4 file type.", nameof(value));
        return fileName;
    }
