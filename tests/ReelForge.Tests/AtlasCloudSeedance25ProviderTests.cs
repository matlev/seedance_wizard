...
public sealed class AtlasCloudSeedance25ProviderTests
{
    ...
    public async Task SubmitTurnsProviderErrorIntoStructuredException()
    {
        ...
        var exception = await Assert.ThrowsAsync<VideoGenerationProviderException>(
            () => provider.SubmitAsync(request, [], TestAuthorization()));
    ...
    [Fact]
    public async Task SubmissionWithoutExplicitAuthorizationCannotReachHttpHandler()
    {
        var handler = new RecordingHandler(HttpStatusCode.OK, "{}");
        var provider = CreateProvider(handler);
        var request = new GenerationRequest
        {
            Prompt = "This must never leave the process",
            Mode = GenerationMode.TextToVideo,
            DurationSeconds = 4,
            AspectRatio = "16:9",
            Resolution = "480p"
    ...
    [Fact]
    public async Task PollReadsVerifiedPredictionSchemaAndHttpsOutputs()
    {
        var handler = new RecordingHandler(
            HttpStatusCode.OK,
            """{"data":{"id":"prediction-123","status":"completed","outputs":["https://storage.example/output.mp4"],"completed_at":"2026-08-10T12:00:00Z"}}""");
        var provider = CreateProvider(handler);
