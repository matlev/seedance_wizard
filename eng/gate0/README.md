...
`g0.5-stage2a-schedule.json` is the committed, order-sensitive 18-cell/108-attempt matrix schedule. `Invoke-G05Stage2AMatrix.ps1` is contract-only by default; its live switch fails closed while `g0.5-stage2a-execution-authorization.json` remains `owner-authorized-execution-implementation-pending`. `Test-G05Stage2AMatrixPreflight.ps1` validates the sealed corpus/index, exact retained P2 runtime closure, reference-host identity, clean media-process state, the approved 3.5 GiB full floor, and positive incremental per-cell reservations. These files do not yet implement or authorize semantic media execution.
