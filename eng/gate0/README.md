...
The canonical current state and active authorization are maintained in `docs/gate-0-current-status.md`. Historical decision/result documents retain the status that was true when their evidence was recorded. Future measured-evidence growth is governed by `docs/gate-0-evidence-containment.md`. The exact Stage 2A matrix and one fresh replacement restart from global ordinal 1 are owner-approved. Containment, the legacy seal, no-media local/R2 proof, deterministic schedule, dependency hash closure, semantic execution, retention integration, and the ordered-dictionary validator repair are complete. Media and P2-route evidence remain fail-closed until the replacement approval binding is committed and a fresh full activation preflight passes.
