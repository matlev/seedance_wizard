...
The canonical current state and active authorization are maintained in `docs/gate-0-current-status.md`. Historical decision/result documents retain the status that was true when their evidence was recorded. Future measured-evidence growth is governed by `docs/gate-0-evidence-containment.md`. The exact Stage 2A matrix and its shard/index containment implementation are authorized; media remains blocked until the legacy seal and no-media local/R2 proof complete.
