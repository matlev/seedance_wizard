...
The canonical current state and active authorization are maintained in `docs/gate-0-current-status.md`. Historical decision/result documents retain the status that was true when their evidence was recorded. Future measured-evidence growth is governed by `docs/gate-0-evidence-containment.md`. The owner classified the six physical attempts from the retained-namespace block as non-authoritative and approved a narrow repair plus one fresh restart. The repair preserves source-relative local validation, projects only the evidence-facing binding into the exact retained destination namespace, and leaves the immutable shard invariant unchanged. Real-helper-to-real-writer positive and negative regressions, the complete 65-test focused no-media set, independent review, fresh full preflight, and complete local/R2 verification pass. One fresh exact Stage 2A restart is effective and must stop after its owner packet.
...
The future-only containment surface lives under `evidence/`. `Seal-Gate0LegacyEvidence.ps1` makes the old manifests immutable only after exact local hashes, a complete fresh R2 retrieval/hash pass, a clean legacy-manifest worktree, and the initial root index all validate. After that seal, the supported legacy append and durable-ledger mutation paths fail closed. `Add-Gate0EvidenceShard.ps1` writes one hash-bound immutable shard plus one ordered root entry under an exclusive lock, enforces the 768 MiB ceiling and metadata caps before remote transfer, and confines payloads to `future/stage2/<proof-run-id>`. A `p2-runtime-route` append additionally requires the exact tracked Stage 2A execution authorization and all of its schedule/runner/preflight/contract hashes; that authorization does not exist until those prerequisites pass. Post-transfer failure journals block further work until `Resolve-Gate0EvidenceAppendJournal.ps1` creates an owner-reviewed failed or blocked shard. `Test-Gate0EvidenceContainment.ps1` performs offline structure/local-byte validation by default and opt-in independent R2 retrieval verification. The root separately reserves two no-media infrastructure shards and the approved 18 Stage 2A cell shards.

`Invoke-Gate0EvidenceContainmentDryRun.ps1` is the only authorized live adoption proof before Stage 2A media. It invokes no FFmpeg/ffprobe process, retains the contract and deterministic byte probe, independently retrieves the exact R2 objects, then retains that validation result in a second immutable infrastructure shard. Hosted CI exercises the structural and negative tests without artifact-root access, credentials, media, or network calls.

`g0.5-stage2a-schedule.json` is the committed, order-sensitive 18-cell/108-attempt matrix schedule. `Invoke-G05Stage2AMatrix.ps1` remains contract-only by default; its live switch requires the exact effective, hash-bound `g0.5-stage2a-execution-authorization.json`. The runner verifies the admitted replacement-smoke closure, applies the complete semantic oracle to every attempt, retains one ordinary complete closure plus every exceptional closure, writes compact passing repeats only after full validation, and stops after Stage 2A. `Test-G05Stage2AMatrixPreflight.ps1` validates the sealed corpus/index, exact retained P2 runtime closure, reference-host identity, clean media-process state, the approved 3.5 GiB full floor, and positive incremental per-cell reservations. The matrix remains P2 runtime-route proof only and grants no Stage 2B, concurrency, long-form, product, shipping-runtime, distribution, or legal authority.
