...
`g0.5-stage2-smoke-preflight-contract.json` proposes the non-media smoke resource gate and records the outstanding owner decision on its numeric floors. It binds the exact three 1080p typical candidates, reference-machine RAM/CPU identity, a proposed 8 GiB available-memory floor, no active media process, non-reparse sibling roots, the approved 0.75 GiB retained ceiling, a proposed equal scratch ceiling, and a proposed 2 GiB fixed disk reserve. `Test-G05Stage2SmokePreflight.ps1` remains fail-closed until the contract records owner approval. Neither file replaces R2 verification, executes media, authorizes the smoke alone, or sizes long-form work.
...
This remains one verified local working copy. The dedicated private `reelforge-artifacts` R2 bucket is now the approved durable target, but the tracked `artifact-manifest.json` remains retention-incomplete until every source-inventory artifact is independently retrieved and hash-verified from R2. `Upload-Gate0Artifact.ps1`, `Get-Gate0Artifact.ps1`, and `Test-Gate0ArtifactManifest.ps1` provide the credential-gated content-addressed transfer and verification surface; `Gate0ArtifactTools.psm1` reads only the three documented dedicated Windows Generic Credentials. Normal CI performs offline validation and receives no credentials. The temporary-provider R2 path remains prohibited for this corpus. See `docs/gate-0-artifact-retention.md`.
