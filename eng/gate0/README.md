...
The canonical current state and active authorization are maintained in `docs/gate-0-current-status.md`. Historical decision/result documents retain the status that was true when their evidence was recorded. Future measured-evidence growth is governed by `docs/gate-0-evidence-containment.md`. The exact Stage 2A matrix is authorized, and its containment, legacy seal, no-media local/R2 proof, deterministic schedule, dependency hash closure, and resource/runtime preflight are complete. The execution authorization remains pending, so media and P2-route evidence remain blocked until the semantic executor and retention integration pass review.
