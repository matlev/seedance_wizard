...
The canonical current state and active authorization are maintained in `docs/gate-0-current-status.md`. Historical decision/result documents retain the status that was true when their evidence was recorded. Future measured-evidence growth is governed by `docs/gate-0-evidence-containment.md`. Stage 2A is fail-closed after the approved replacement run exposed a runner-to-writer retained-namespace integration defect during first-cell candidate-shard validation. The six new physical attempts remain local and non-authoritative pending owner disposition; no R2, payload, shard, or root-index mutation occurred. The exact validator invariant and frozen matrix remain unchanged, and no further media is authorized.
