...
The canonical current state and active authorization are maintained in `docs/gate-0-current-status.md`. Historical decision/result documents retain the status that was true when their evidence was recorded. Future measured-evidence growth is governed by `docs/gate-0-evidence-containment.md`. The exact Stage 2A matrix and one fresh replacement restart from global ordinal 1 are owner-approved and effective after the focused no-media suite, independent review, fresh full activation preflight, and complete local/R2 byte verification passed. Containment, the legacy seal, deterministic schedule, dependency hash closure, semantic execution, retention integration, and the ordered-dictionary validator repair are complete. The effective authority remains limited to the unchanged Stage 2A matrix and owner packet.
