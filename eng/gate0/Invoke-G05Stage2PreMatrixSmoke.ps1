...
function Sanitize-G05Text([string] $Value, [hashtable] $Roots) {
    $result=$Value
    foreach($entry in $Roots.GetEnumerator()){$root=[string]$entry.Value;$replacement="{$($entry.Key)}";$result=$result.Replace($root,$replacement,[StringComparison]::OrdinalIgnoreCase);$result=$result.Replace($root.Replace('\','/'),$replacement,[StringComparison]::OrdinalIgnoreCase)}
    $result
}

$state=[ordered]@{schemaVersion=2;schemaId='Gate0.G05.Stage2.ReplacementPreMatrixSmoke.V2';status='started';startedUtc=[DateTimeOffset]::UtcNow;completedUtc=$null;contractOnly=[bool]$ContractOnly;manualExecution=[bool]$ManualExecution;appendRetention=[bool]$AppendRetention;evidenceBoundary='runtime-route';noMediaExecuted=$true;preflight=[ordered]@{};contracts=[ordered]@{};candidates=@();attempts=@();retention=$null;error=$null;nonClaims=@('No current ReelForge product render, WPF, preview, cache, cancellation, or project behavior claim.','No shipping-runtime, distribution, patent, legal, public hardware-floor, full-matrix, or long-form conclusion.')}
...
try {
    ...
    $authorizationPath=Join-Path $PSScriptRoot 'g0.5-stage2-replacement-smoke-authorization-summary.json';$amendmentPath=Join-Path $PSScriptRoot 'g0.5-lossy-audio-oracle-amendment-v4.json';$freezePath=Join-Path $PSScriptRoot 'g0.5-lossy-audio-oracle-amendment-v4-freeze.json';$controlSummaryPath=Join-Path $PSScriptRoot 'g0.5-structured-audio-control-result-summary.json';$controlRetentionSummaryPath=Join-Path $PSScriptRoot 'g0.5-structured-audio-control-retention-result-summary.json';$p2ManifestPath=Join-Path $PSScriptRoot 'manifests/p2-btbn-lgplv3-shared-windows-x64-20260820.json';$trackedRetentionManifest=Join-Path $PSScriptRoot 'artifact-retention-manifest.json';$durableArtifactManifest=Join-Path $PSScriptRoot 'artifact-manifest.json';$localRetentionManifest=Join-Path $artifact 'artifact-retention-manifest.json'
    $snapshotSources=@{'workload-contract.json'=$contractPath;'audio-oracle-contract.json'=$audioContractPath;'audio-oracle-amendment-v4.json'=$amendmentPath;'audio-oracle-amendment-v4-freeze.json'=$freezePath;'structured-audio-control-summary.json'=$controlSummaryPath;'structured-audio-control-retention-summary.json'=$controlRetentionSummaryPath;'replacement-smoke-authorization-summary.json'=$authorizationPath;'smoke-helper.psm1'=$helperPath;'audio-timing-helper.psm1'=$audioTimingHelperPath;'smoke-runner.ps1'=$PSCommandPath;'retention-appender.ps1'=$retentionAppender;'retention-validator.ps1'=$retentionValidator;'runtime-validator.ps1'=$runtimeValidator;'p2-runtime-manifest.json'=$p2ManifestPath;'tracked-retention-manifest.json'=$trackedRetentionManifest;'durable-artifact-manifest.json'=$durableArtifactManifest;'local-retention-manifest.json'=$localRetentionManifest}
    ...
    $snapshotWorkload=Join-Path $snapshots 'workload-contract.json';$snapshotAudioContract=Join-Path $snapshots 'audio-oracle-contract.json';$snapshotAmendment=Join-Path $snapshots 'audio-oracle-amendment-v4.json';$snapshotFreeze=Join-Path $snapshots 'audio-oracle-amendment-v4-freeze.json';$snapshotControlSummary=Join-Path $snapshots 'structured-audio-control-summary.json';$snapshotControlRetentionSummary=Join-Path $snapshots 'structured-audio-control-retention-summary.json';$snapshotAuthorization=Join-Path $snapshots 'replacement-smoke-authorization-summary.json';$snapshotDurableManifest=Join-Path $snapshots 'durable-artifact-manifest.json';$snapshotHelper=Join-Path $snapshots 'smoke-helper.psm1';$snapshotAudioTimingHelper=Join-Path $snapshots 'audio-timing-helper.psm1';$snapshotP2Manifest=Join-Path $snapshots 'p2-runtime-manifest.json'
    ...
    $authorization=Get-Content $snapshotAuthorization -Raw|ConvertFrom-Json -Depth 32;$amendment=Get-Content $snapshotAmendment -Raw|ConvertFrom-Json -Depth 32;$freeze=Get-Content $snapshotFreeze -Raw|ConvertFrom-Json -Depth 32;$control=Get-Content $snapshotControlSummary -Raw|ConvertFrom-Json -Depth 32;$controlRetention=Get-Content $snapshotControlRetentionSummary -Raw|ConvertFrom-Json -Depth 32
    if($authorization.status-ne'passed-controls-retention-and-resource-preflight-replacement-smoke-authorized'-or-not$authorization.gates.replacementPreMatrixSmokeAuthorized-or$authorization.gates.fullStage2MatrixAuthorized-or$authorization.gates.wpfMediaScenariosAuthorized-or$authorization.gates.concurrencyComparisonAuthorized-or$authorization.gates.longFormAuthorized){throw'Replacement smoke authorization is unavailable or widened.'};$authorizedIds=@($authorization.authorizedCandidates|ForEach-Object candidateId|Sort-Object);$expandedIds=@($rows|ForEach-Object candidateId|Sort-Object);if($authorizedIds.Count-ne3-or@($authorization.authorizedCandidates|Where-Object{$_.attemptCount-ne1}).Count-or-not([string]::Join('|',$authorizedIds)-eq[string]::Join('|',$expandedIds))){throw'Replacement authorization must contain exactly the three expanded one-attempt candidates.'};if($amendment.amendmentId-ne'Gate0.G05.LossyAudioOracle.V4.ReferenceRelativeTypical.20260827'-or$amendment.extends.sha256-ne(Get-G05SmokeHash $snapshotAudioContract)-or$freeze.v3Contract.sha256-ne(Get-G05SmokeHash $snapshotAudioContract)-or$freeze.v4Amendment.sha256-ne(Get-G05SmokeHash $snapshotAmendment)-or$freeze.freezeId-ne$authorization.oracle.freezeId-or$control.freezeId-ne$freeze.freezeId-or$control.status-ne'passed-controls-only-retention-pending'-or$controlRetention.status-ne'complete'-or$controlRetention.controlGroup.groupId-ne$authorization.oracle.controlGroupId-or$controlRetention.freeze.retainedSnapshotSha256-ne$authorization.oracle.freezeSha256-or$control.structuredControls.Count-ne5-or$control.legacyV3Controls.count-ne12-or-not$control.legacyV3Controls.allFrozenHashesAndDispositionsPreserved-or-not$control.legacyV3Controls.allNoOverlayEffectiveOracleDispositionsPreserved-or$control.executionBoundaries.routeOutputsEvaluated-or$freeze.controlEvidence.routeOutputsEvaluated-or$freeze.controlEvidence.structuredControlCount-ne5-or$freeze.controlEvidence.legacyV3ControlCount-ne12-or-not$freeze.controlEvidence.allDeclaredDispositionsPassed-or-not$freeze.controlEvidence.legacyHashesAndDispositionsPreserved-or-not$authorization.oracle.controlClosureLocalAndR2Verified-or$authorization.oracle.routeOutputsEvaluatedBeforeFreezeAndRetention-or(Get-G05SmokeHash $snapshotFreeze)-ne$authorization.oracle.freezeSha256-or$control.controlReport.sha256-ne$authorization.oracle.controlReportSha256-or$controlRetention.controlGroup.controlReportSha256-ne$authorization.oracle.controlReportSha256-or$freeze.controlEvidence.sha256-ne$authorization.oracle.controlReportSha256-or@($freeze.implementation|Where-Object{$_.path-eq'eng/gate0/G05Stage2SmokeHelpers.psm1' -and $_.sha256-eq(Get-G05SmokeHash $snapshotHelper)}).Count-ne1){throw'Frozen structured audio control closure does not authorize route evaluation.'}
    $retentionValidatorHash=Get-G05SmokeSnapshotHash $state.contracts.snapshot.bindings 'retention-validator.ps1';if((Get-G05SmokeHash $retentionValidator)-ne$retentionValidatorHash){throw'Retention validator changed after snapshot binding.'};$local=&$retentionValidator -ArtifactRoot $artifact;if((Get-G05SmokeHash $retentionValidator)-ne$retentionValidatorHash){throw'Retention validator changed during corpus validation.'};$sealed=$authorization.sealedCorpusAfterPreflightRetention;$durable=Get-Content $snapshotDurableManifest -Raw|ConvertFrom-Json -Depth 100;if($local.status-ne'verified'-or$local.manifestSha256-ne$sealed.sourceManifestSha256-or$local.fileCount-ne$sealed.logicalArtifactCount-or$local.totalBytes-ne$sealed.logicalArtifactBytes-or(Get-G05SmokeHash $snapshotDurableManifest)-ne$sealed.durableManifestSha256-or$durable.sourceInventory.sha256-ne$sealed.sourceManifestSha256-or$durable.sourceInventory.logicalArtifactCount-ne$sealed.logicalArtifactCount-or$durable.sourceInventory.logicalArtifactBytes-ne$sealed.logicalArtifactBytes-or$durable.status.retentionCondition-ne$sealed.retentionCondition-or-not$durable.status.secondPrivateCopyVerified-or$durable.status.verifiedLogicalArtifactCount-ne$sealed.recordedRemoteVerifiedLogicalArtifacts-or$durable.status.verifiedLogicalArtifactBytes-ne$sealed.logicalArtifactBytes-or@($durable.artifacts|Where-Object{$_.retentionStatus-ne'remote-verified'}).Count){throw'Current local or durable corpus no longer matches the sealed replacement-smoke authorization binding.'};$manifest=Get-Content $localRetentionManifest -Raw|ConvertFrom-Json -Depth 100;$controlGroup=@($manifest.groups|Where-Object{$_.groupId-eq$controlRetention.controlGroup.groupId});if($controlGroup.Count-ne1-or-not(@($controlGroup[0].files|Where-Object{$_.filename-eq$control.controlReport.logicalId-and$_.sha256-eq$control.controlReport.sha256-and$_.size-eq$control.controlReport.size}).Count-eq1)){throw'Retained structured audio control evidence is absent or mismatched.'};$preflightGroup=@($manifest.groups|Where-Object{$_.groupId-eq$authorization.resourcePreflight.evidenceGroupId});if($preflightGroup.Count-ne1-or-not(@($preflightGroup[0].files|Where-Object{$_.filename-eq$authorization.resourcePreflight.evidenceLogicalId-and$_.sha256-eq$authorization.resourcePreflight.evidenceSha256-and$_.size-eq$authorization.resourcePreflight.evidenceSize}).Count-eq1)){throw'Retained replacement resource-preflight evidence is absent or mismatched.'};$state.preflight=[ordered]@{localCorpus=$local;replacementAuthorizationSha256=(Get-G05SmokeHash $snapshotAuthorization);durableArtifactManifestSha256=(Get-G05SmokeHash $snapshotDurableManifest);authorizationId=$authorization.authorizationId;authorization='bounded-replacement-pre-matrix-smoke-only'}
    Assert-G05NoActiveMedia
    $runtimeValidatorHash=Get-G05SmokeSnapshotHash $state.contracts.snapshot.bindings 'runtime-validator.ps1';if((Get-G05SmokeHash $runtimeValidator)-ne$runtimeValidatorHash){throw'Runtime validator changed after snapshot binding.'};$runtimeRaw=Join-Path $stage 'runtime-identity.raw.json';&$runtimeValidator -RuntimeRoot $runtime -ManifestPath $snapshotP2Manifest -EvidencePath $runtimeRaw|Out-Null;if((Get-G05SmokeHash $runtimeValidator)-ne$runtimeValidatorHash){throw'Runtime validator changed during runtime validation.'};$runtimeText=Get-Content $runtimeRaw -Raw;$runtimeText=Sanitize-G05Text $runtimeText @{artifactRoot=$artifact;runtimeRoot=$runtime;repositoryRoot=$repositoryRoot;stage=$stage};[IO.File]::WriteAllText((Join-Path $stage 'runtime-identity.json'),$runtimeText,[Text.UTF8Encoding]::new($false));Remove-Item $runtimeRaw -Force
    if((Get-G05SmokeHash $snapshotAudioContract)-ne'119A4C179BFA010F3202DBF6AA368E42EDE5FD0FC23EF2781AA9C7F63540CBE4'){throw'Frozen audio oracle contract SHA-256 mismatch.'};$audioContract=Get-Content $snapshotAudioContract -Raw|ConvertFrom-Json -Depth 100;$descriptor=@($audioContract.referenceDescriptors|Where-Object{$_.id -eq 'typical-2v4a-30s'});$overlays=@($amendment.descriptorOverlays|Where-Object{$_.referenceDescriptorId-eq'typical-2v4a-30s'});if($descriptor.Count-ne1-or$overlays.Count-ne1){throw'Frozen typical audio descriptor or V4 overlay is unavailable.'};$overlay=$overlays[0];Assert-G05SmokeReferenceRelativeOverlay $descriptor[0] $overlay;$state.contracts.audioOracle=[ordered]@{path='snapshots/audio-oracle-contract.json';sha256=(Get-G05SmokeHash $snapshotAudioContract);contractId=$audioContract.contractId};$state.contracts.audioOracleV4=[ordered]@{path='snapshots/audio-oracle-amendment-v4.json';sha256=(Get-G05SmokeHash $snapshotAmendment);amendmentId=$amendment.amendmentId;overlayMode=$overlay.mode};$state.contracts.audioOracleFreeze=[ordered]@{path='snapshots/audio-oracle-amendment-v4-freeze.json';sha256=(Get-G05SmokeHash $snapshotFreeze);freezeId=$freeze.freezeId};$state.contracts.structuredControls=[ordered]@{resultPath='snapshots/structured-audio-control-summary.json';resultSha256=(Get-G05SmokeHash $snapshotControlSummary);retentionPath='snapshots/structured-audio-control-retention-summary.json';retentionSha256=(Get-G05SmokeHash $snapshotControlRetentionSummary);controlGroupId=$controlRetention.controlGroup.groupId};$state.contracts.harness=[ordered]@{path='snapshots/smoke-runner.ps1';sha256=(Get-G05SmokeHash (Join-Path $snapshots 'smoke-runner.ps1'))};$state.contracts.helper=[ordered]@{path='snapshots/smoke-helper.psm1';sha256=(Get-G05SmokeHash $snapshotHelper)};$state.contracts.audioTimingHelper=[ordered]@{path='snapshots/audio-timing-helper.psm1';sha256=(Get-G05SmokeHash $snapshotAudioTimingHelper)}
    $truth=New-G05TypicalAudioTruth $fixture $rows[0].workload (Join-Path $stage 'typical-2v4a-30s.s16le');$identityOracle=Test-G05SmokeAudio $truth $truth $audioContract.qualityThresholds $descriptor[0] 1024 $overlay;$state.preflight.audioOracleIdentity=$identityOracle;if(-not$identityOracle.passed){throw('The frozen audio oracle rejects its own workload reference: '+($identityOracle.failures-join','))};$ffmpeg=Join-Path $runtime 'bin/ffmpeg.exe';$ffprobe=Join-Path $runtime 'bin/ffprobe.exe'
    $portableRoots=@{artifactRoot=$artifact;runtimeRoot=$runtime;fixtureRoot=$fixture;stage=$stage}
    foreach($row in $rows){
        try { Assert-G05NoActiveMedia }
        catch { $state.preflight.candidateStartContamination=[ordered]@{candidateId=$row.candidateId;status='harness-definition-stop-before-candidate-media';message=(Sanitize-G05Text $_.Exception.Message $portableRoots)};throw "Active media contamination blocks candidate $($row.candidateId) before execution." }
        ...
        try{
            ...
            $record.descriptor=Test-G05Descriptor $probe $row.route (Get-G05Descriptor $contract $row.route);$videoFrames=@($probe.frames|Where-Object{$_.media_type -eq 'video'});$record.videoTiming=Get-G05VideoTiming $record.descriptor.videoStream $videoFrames
            ...
            $audioFrames=@($probe.frames|Where-Object{$_.media_type -eq 'audio'});$audioPackets=@($packets.packets|Where-Object{$_.codec_type -eq 'audio'});$timing=Get-G05DecodedAudioTiming -RawByteLength (Get-Item $pcm).Length -AudioStream $record.descriptor.audioStream -AudioFrames $audioFrames -AudioPackets $audioPackets -ExpectedSamplesPerChannel 1440000 -MaximumRawTailSamples 1024;$quality=Test-G05SmokeAudio $truth $pcm $audioContract.qualityThresholds $descriptor[0] 1024 $overlay;$onsets=@($quality.onsetTiming|ForEach-Object{[ordered]@{id=$_.Id;expectedStartSample=$_.ExpectedStartSample;expectedStartSeconds=$_.ExpectedStartSeconds;observedStartSample=$_.ObservedStartSample;observedStartSeconds=$_.ObservedStartSeconds;signedErrorOffsetSamples=$_.SignedErrorOffsetSamples;signedErrorOffsetSeconds=$_.SignedErrorOffsetSeconds;passed=$_.Passed}});$record.audio=[ordered]@{timing=$timing;quality=$quality;onsetEvidence=$onsets;rawPcm=(Get-G05FileEvidence $pcm $stage)};if(-not$timing.passed){throw("Audio timing failed: "+($timing.failures-join','))};if(-not$quality.passed){throw("Audio quality failed: "+($quality.failures-join','))}
            ...
            $record.failures+=,(Sanitize-G05Text $_.Exception.Message $portableRoots);if($null-ne$encodeResult-and$encodeResult.exitCode-eq0-and(Test-Path $output -PathType Leaf)){$record.media=Get-G05FileEvidence $output $stage;$record.cleanup=[ordered]@{partialOutput='not-partial-complete-output-retained-for-failed-oracle';orphanFree=$encodeResult.processTree.orphanFree}}else{$record.cleanup=Remove-G05PartialOutput $output $stage};$record.status='failed'
        }finally{try{Assert-G05NoActiveMedia}catch{$record.failures+=,(Sanitize-G05Text $_.Exception.Message $portableRoots);$record.status='failed'};$record.completedUtc=[DateTimeOffset]::UtcNow}
    }
    $state.status=if(@($state.attempts|Where-Object{$_.status -eq 'failed'}).Count){'completed-with-failures'}elseif(@($state.attempts|Where-Object{$_.status -eq 'passed'}).Count-eq3){'completed'}else{'infrastructure-failed'}
}catch{$state.status='harness-definition-failed';$state.error=if($null-ne$artifact){Sanitize-G05Text $_.Exception.Message @{artifactRoot=$artifact;repositoryRoot=$repositoryRoot;stage=$stage}}else{$_.Exception.Message}}
finally{
    ...
    if($retentionAvailable-and$null-ne$stage-and(Test-Path $stage -PathType Container)){
        ...
        else{try{$appenderHash=Get-G05SmokeSnapshotHash $state.contracts.snapshot.bindings 'retention-appender.ps1';$validatorHash=Get-G05SmokeSnapshotHash $state.contracts.snapshot.bindings 'retention-validator.ps1';if((Get-G05SmokeHash $retentionAppender)-ne$appenderHash-or(Get-G05SmokeHash $retentionValidator)-ne$validatorHash){throw'Retention tooling changed after snapshot binding.'};&$retentionAppender -ArtifactRoot $artifact -SourceRoot $stage -SourceTrustBoundary $projectParent -GroupId $group -DestinationName $destination -Provenance 'Manual Gate 0 G0.5 authorized replacement pre-matrix smoke attempts; every candidate disposition retained.' -ProducerRuntimeIdentity @("artifact:$destination/snapshots/smoke-runner.ps1","artifact:$destination/snapshots/smoke-helper.psm1","artifact:$destination/snapshots/p2-runtime-manifest.json") -LicenseRecords @('artifact:p2/runtime/ffmpeg-n8.1.2-44-g7c533d0f86-win64-lgpl-shared-8.1/LICENSE.txt') -ProofRunIdentity @("artifact:$destination/g0.5-stage2-pre-matrix-smoke-evidence.json");if((Get-G05SmokeHash $retentionAppender)-ne$appenderHash-or(Get-G05SmokeHash $retentionValidator)-ne$validatorHash){throw'Retention tooling changed during append.'};&$retentionValidator -ArtifactRoot $artifact|Out-Null;$state.retention.status='appended-and-validated';$exitCode=if($state.status-eq'completed'){0}else{1}}catch{$state.status='retention-failed';$state.retention.status='append-or-validation-failed';$state.retention.error=$_.Exception.GetType().Name;Write-G05AtomicJson $evidence $state}}
    }
