...
namespace ReelForge.Gate0 {
  ...
  public sealed class SmokeOnsetTimingMetric { public string Id { get; set; } = ""; public int ExpectedStartSample { get; set; } public double ExpectedStartSeconds { get; set; } public int ObservedOffsetSamples { get; set; } public double ObservedOffsetSeconds { get; set; } public int SignedErrorOffsetSamples { get; set; } public double SignedErrorOffsetSeconds { get; set; } public int ObservedStartSample { get; set; } public double ObservedStartSeconds { get; set; } public double BestMeanSquaredError { get; set; } public bool Passed { get; set; } public string Method { get; set; } = ""; }
  public sealed class SmokeReferenceRelativeWindowMetric { public string Id { get; set; } = ""; public int StartSample { get; set; } public int EndSampleExclusive { get; set; } public int WindowSamples { get; set; } public int CandidateWindowCount { get; set; } public int ActiveWindowCount { get; set; } public double MinimumReferenceRms { get; set; } public double MaximumReferenceRms { get; set; } public double MinimumOutputRms { get; set; } public double MaximumOutputRms { get; set; } public double MinimumOutputToReferenceRmsRatio { get; set; } public double MaximumOutputToReferenceRmsRatio { get; set; } public int[] FailingWindowStartSamples { get; set; } = Array.Empty<int>(); public string[] Failures { get; set; } = Array.Empty<string>(); public bool Passed { get; set; } }
  public static class SmokeAudioOracle {
    public static SmokeAudioRegionMetric Analyze(string id, short[] reference, short[] actual, int start, int end, int[] expectedLeft, int[] expectedRight, int[] forbiddenLeft, int[] forbiddenRight,
      ...
      double minToneDominance, double minActiveRms, int silenceWindow, double minWindowRms, int clipThreshold, int maxClipped, bool referenceRelativeActivity) {
      ...
      for (int channel = 0; channel < 2; channel++) {
        ...
        if(!(correlation>=minCorrelation))failures.Add($"{id}:channel-{channel}:correlation");if(!(nrmse<=maxNrmse))failures.Add($"{id}:channel-{channel}:nrmse");if(!(snr>=minSnr))failures.Add($"{id}:channel-{channel}:snr");if(!(rmsRatio>=minRmsRatio&&rmsRatio<=maxRmsRatio))failures.Add($"{id}:channel-{channel}:rms-ratio");if(!(dc<=maxDc))failures.Add($"{id}:channel-{channel}:dc-offset");if(!referenceRelativeActivity&&!(activeRms>=minActiveRms))failures.Add($"{id}:channel-{channel}:active-rms");if(!referenceRelativeActivity&&!(minimumWindow>=minWindowRms))failures.Add($"{id}:channel-{channel}:active-window-silence");if(clipped>maxClipped)failures.Add($"{id}:channel-{channel}:near-clipping");
    ...
    public static SmokeOnsetTimingMetric LocateTransition(string id, short[] reference, short[] actual, int start, int searchRadius, int windowRadius) {
      ...
      if(start==0)return new SmokeOnsetTimingMetric{Id=id,ExpectedStartSample=0,ExpectedStartSeconds=0,ObservedOffsetSamples=0,ObservedOffsetSeconds=0,SignedErrorOffsetSamples=0,SignedErrorOffsetSeconds=0,ObservedStartSample=0,ObservedStartSeconds=0,BestMeanSquaredError=0,Passed=true,Method="content-normalized-stream-start"};
      ...
      return new SmokeOnsetTimingMetric{Id=id,ExpectedStartSample=start,ExpectedStartSeconds=start/48000.0,ObservedOffsetSamples=bestOffset,ObservedOffsetSeconds=bestOffset/48000.0,SignedErrorOffsetSamples=bestOffset,SignedErrorOffsetSeconds=bestOffset/48000.0,ObservedStartSample=start+bestOffset,ObservedStartSeconds=(start+bestOffset)/48000.0,BestMeanSquaredError=best,Passed=bestOffset==0,Method="minimum-stereo-waveform-error-over-independent-reference-plus-or-minus-512-samples"};
    }
    public static SmokeReferenceRelativeWindowMetric EvaluateReferenceRelativeWindows(string id, short[] reference, short[] actual, int start, int end, int window, double minRatio, double maxRatio) {
      if(reference.Length!=actual.Length||reference.Length%2!=0)throw new InvalidOperationException("Reference-relative window arrays must be equal stereo interleaved samples.");
      if(window!=960)throw new InvalidOperationException("Reference-relative window length must be exactly 960 samples.");
      int frames=reference.Length/2;if(start<0||end>frames||end-start<window)throw new InvalidOperationException("Reference-relative window region is invalid.");
      int candidate=0,active=0;double minRef=double.PositiveInfinity,maxRef=double.NegativeInfinity,minOut=double.PositiveInfinity,maxOut=double.NegativeInfinity,minObservedRatio=double.PositiveInfinity,maxObservedRatio=double.NegativeInfinity;var failed=new List<int>();var failures=new HashSet<string>();
      for(int channel=0;channel<2;channel++) { double refSum=0,outSum=0; for(int n=start;n<start+window;n++){double x=reference[n*2+channel],y=actual[n*2+channel];refSum+=x*x;outSum+=y*y;} for(int offset=start;;offset++) { candidate++; double refRms=Math.Sqrt(refSum/window)/32768.0,outRms=Math.Sqrt(outSum/window)/32768.0; if(!double.IsFinite(refRms)){failures.Add($"{id}:channel-{channel}:non-finite-reference-rms");if(failed.Count<32)failed.Add(offset);} else if(refRms>0) {active++;minRef=Math.Min(minRef,refRms);maxRef=Math.Max(maxRef,refRms);minOut=Math.Min(minOut,outRms);maxOut=Math.Max(maxOut,outRms);double ratio=outRms/refRms;minObservedRatio=Math.Min(minObservedRatio,ratio);maxObservedRatio=Math.Max(maxObservedRatio,ratio);if(!double.IsFinite(outRms)||!double.IsFinite(ratio)||ratio<minRatio||ratio>maxRatio){failures.Add($"{id}:channel-{channel}:reference-relative-window-rms-ratio");if(failed.Count<32)failed.Add(offset);}} if(offset+window>=end)break; double addRef=reference[(offset+window)*2+channel],removeRef=reference[offset*2+channel],addOut=actual[(offset+window)*2+channel],removeOut=actual[offset*2+channel];refSum+=addRef*addRef-removeRef*removeRef;outSum+=addOut*addOut-removeOut*removeOut; }
      }
      if(active==0)failures.Add($"{id}:no-active-reference-windows");
      return new SmokeReferenceRelativeWindowMetric{Id=id,StartSample=start,EndSampleExclusive=end,WindowSamples=window,CandidateWindowCount=candidate,ActiveWindowCount=active,MinimumReferenceRms=minRef,MaximumReferenceRms=maxRef,MinimumOutputRms=minOut,MaximumOutputRms=maxOut,MinimumOutputToReferenceRmsRatio=minObservedRatio,MaximumOutputToReferenceRmsRatio=maxObservedRatio,FailingWindowStartSamples=failed.ToArray(),Failures=new List<string>(failures).ToArray(),Passed=failures.Count==0};
    }
...
function Invoke-G05SmokeAudioRegion([string] $Id, [int16[]] $Reference, [int16[]] $Actual, [int] $Start, [int] $End, [object] $ExpectedFrequencies, [object] $ForbiddenFrequencies, [object] $Thresholds, [object] $ReferenceRelativeOverlay = $null) {
    ...
    $referenceRelativeActivity=$null-ne$ReferenceRelativeOverlay
    [ReelForge.Gate0.SmokeAudioOracle]::Analyze($Id,$Reference,$Actual,$Start,$End,$leftExpected,$rightExpected,$leftForbidden,$rightForbidden,[double]$Thresholds.minimumSignedNormalizedCrossCorrelationPerChannel,[double]$Thresholds.maximumNormalizedRmsErrorPerChannel,[double]$Thresholds.minimumSnrDbPerChannel,[double]$Thresholds.minimumOutputToReferenceRmsRatioPerChannel,[double]$Thresholds.maximumOutputToReferenceRmsRatioPerChannel,[double]$Thresholds.maximumAbsoluteDcOffsetFullScalePerChannel,[double]$Thresholds.minimumExpectedToneOutputToReferenceAmplitudeRatio,[double]$Thresholds.maximumExpectedToneOutputToReferenceAmplitudeRatio,[double]$Thresholds.minimumExpectedToForbiddenTonePowerRatioWhenDescriptorProvidesForbiddenTones,[double]$Thresholds.minimumActiveChannelRmsFullScale,[int]$Thresholds.silenceWindowSamples,[double]$Thresholds.minimumActiveReferenceWindowOutputRmsFullScale,[int]$Thresholds.nearClippingSampleAbsoluteThreshold,[int]$Thresholds.maximumUnexpectedNearClippedSamplesPerChannel,$referenceRelativeActivity)
}

function Assert-G05SmokeReferenceRelativeOverlay([object] $Descriptor, [object] $Overlay) {
    if ([string]$Descriptor.id -ne 'typical-2v4a-30s') { throw 'Reference-relative overlay is approved only for typical-2v4a-30s.' }
    if ([string]$Overlay.referenceDescriptorId -ne [string]$Descriptor.id -or [string]$Overlay.mode -ne 'reference-relative-active-windows-v1') { throw 'Reference-relative overlay descriptor binding or mode is invalid.' }
    if ([int]$Overlay.sampleRate -ne 48000 -or [int]$Overlay.windowSamples -ne 960) { throw 'Reference-relative overlay sample rate or window length is invalid.' }
    if ([double]$Overlay.minimumOutputToReferenceRmsRatio -ne 0.90 -or [double]$Overlay.maximumOutputToReferenceRmsRatio -ne 1.10) { throw 'Reference-relative overlay RMS-ratio bounds are invalid.' }
    $replacements = @($Overlay.replacesV3Checks | ForEach-Object { [string]$_ } | Sort-Object -Unique)
    ...
    if ($replacements.Count -ne 2 -or @($replacements | Where-Object { $_ -notin $required }).Count -ne 0 -or @($required | Where-Object { $_ -notin $replacements }).Count -ne 0) { throw 'Reference-relative overlay replacement set is invalid.' }
}

function Test-G05SmokeAudio([string] $Reference, [string] $Actual, [object] $Thresholds, [object] $Descriptor, [int] $MaximumRawTailSamples = 1024, [object] $ReferenceRelativeOverlay = $null) {
    Initialize-G05SmokeAudioOracle
    if ($null -ne $ReferenceRelativeOverlay) { Assert-G05SmokeReferenceRelativeOverlay $Descriptor $ReferenceRelativeOverlay }
    ...
    $forbiddenProperty=$Descriptor.PSObject.Properties['forbiddenFrequenciesHzByChannel'];$forbidden=if($null-eq$forbiddenProperty){$null}else{$forbiddenProperty.Value};$regions=[Collections.Generic.List[object]]::new();$relativeRegions=[Collections.Generic.List[object]]::new();$fullStart=2048;$fullEnd=([int]$Descriptor.samplesPerChannel-2048);$regions.Add((Invoke-G05SmokeAudioRegion 'full-quality-region' $referenceSamples $actualSamples $fullStart $fullEnd $Descriptor.activeExpectedFrequenciesHzByChannel $forbidden $Thresholds $ReferenceRelativeOverlay));if($null-ne$ReferenceRelativeOverlay){$relativeRegions.Add([ReelForge.Gate0.SmokeAudioOracle]::EvaluateReferenceRelativeWindows('full-quality-region',$referenceSamples,$actualSamples,$fullStart,$fullEnd,[int]$ReferenceRelativeOverlay.windowSamples,[double]$ReferenceRelativeOverlay.minimumOutputToReferenceRmsRatio,[double]$ReferenceRelativeOverlay.maximumOutputToReferenceRmsRatio))}
    $trackOnsetProperty=$Descriptor.PSObject.Properties['trackOnsetWindows'];$trackOnsetWindows=if($null-eq$trackOnsetProperty){@()}else{@($trackOnsetProperty.Value)};$onsetTiming=[Collections.Generic.List[object]]::new();foreach($window in $trackOnsetWindows){$start=[int]$window.startSample+512;$end=[int]$window.endSampleExclusive-512;$regions.Add((Invoke-G05SmokeAudioRegion ([string]$window.id) $referenceSamples $actualSamples $start $end $window.expectedFrequenciesHzByChannel $null $Thresholds $ReferenceRelativeOverlay));if($null-ne$ReferenceRelativeOverlay){$relativeRegions.Add([ReelForge.Gate0.SmokeAudioOracle]::EvaluateReferenceRelativeWindows([string]$window.id,$referenceSamples,$actualSamples,$start,$end,[int]$ReferenceRelativeOverlay.windowSamples,[double]$ReferenceRelativeOverlay.minimumOutputToReferenceRmsRatio,[double]$ReferenceRelativeOverlay.maximumOutputToReferenceRmsRatio))};$onsetTiming.Add([ReelForge.Gate0.SmokeAudioOracle]::LocateTransition([string]$window.id,$referenceSamples,$actualSamples,[int]$window.startSample,512,2048))}
    $qualityFailures=@($regions|ForEach-Object{@($_.Failures)})+@($relativeRegions|ForEach-Object{@($_.Failures)});$timingFailures=@($onsetTiming|Where-Object{-not$_.Passed}|ForEach-Object{"$($_.Id):onset-offset-$($_.ObservedOffsetSamples)"});$failures=@($qualityFailures)+@($timingFailures);[ordered]@{passed=$failures.Count-eq0;qualityPassed=$qualityFailures.Count-eq0;onsetTimingPassed=$timingFailures.Count-eq0;failures=$failures;regions=@($regions);referenceRelativeRegions=@($relativeRegions);referenceRelativeActivityEnabled=($null-ne$ReferenceRelativeOverlay);onsetTiming=@($onsetTiming);rawPcm=[ordered]@{bytes=$actualBytes.Length;sha256=(Get-G05SmokeHash $Actual);samplesPerChannel=$actualBytes.Length/4;tailSamples=($actualBytes.Length-$expectedBytes)/4};contentNormalized=[ordered]@{selection='first descriptor.samplesPerChannel samples after decoder-applied skip/discard';bytes=$expectedBytes;sha256=([Convert]::ToHexString([Security.Cryptography.SHA256]::HashData($actualContentBytes)));proofSideTrimmingPerformed=$false}}
}
