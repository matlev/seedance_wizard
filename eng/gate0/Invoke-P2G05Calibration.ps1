...
function Invoke-RequiredScript([string] $Path,[hashtable] $Arguments) { & $Path @Arguments }
...
function Get-ProbedVideoFrames([object] $Parsed) {
    $combined=Get-Property $Parsed 'packets_and_frames' $null
    if($null-eq$combined){throw 'Combined packet/frame inspection oracle is missing.'}
    $frames=@($combined|Where-Object{$_.type-eq'frame'-and$_.media_type-eq'video'})
    if($frames.Count-eq0){throw 'Combined packet/frame inspection contains no video frames.'}
    @($frames)
}
...
function New-FfmpegArguments([object] $Row, [object] $Route, [object] $Contract, [string] $FixtureRoot, [string] $Output, [int] $Duration) {
    ...
    $commandArguments=[Collections.Generic.List[string]]::new(); @('-nostdin','-hide_banner','-progress','pipe:1','-stats_period','0.5','-stream_loop','-1','-framerate','25','-f','image2','-c:v','ppm','-i',$sequence,'-stream_loop','-1','-f','s16le','-c:a','pcm_s16le','-ar','48000','-ac','2','-i',$pcm) | ForEach-Object { $commandArguments.Add($_) }; if($null -ne $Row.filterThreads){$commandArguments.Add('-filter_threads');$commandArguments.Add([string]$Row.filterThreads)};$commandArguments.Add('-filter:v');$commandArguments.Add("scale=$($Row.width):$($Row.height):flags=bilinear,format=yuv420p,setpts=PTS-STARTPTS");$commandArguments.Add('-filter:a');$commandArguments.Add('aresample=48000,aformat=sample_fmts=s16:sample_rates=48000:channel_layouts=stereo,asetpts=PTS-STARTPTS');@('-map','0:v:0','-map','1:a:0','-threads:v',[string]$Row.codecThreads,'-c:v',$Route.videoEncoder) | ForEach-Object {$commandArguments.Add($_)}; foreach($option in @($Route.videoOptions)){$commandArguments.Add([string]$option)};@('-c:a',$Route.audioEncoder) | ForEach-Object {$commandArguments.Add($_)};foreach($option in @($Route.audioOptions)){$commandArguments.Add([string]$option)};foreach($option in @(Get-Property $Route 'muxerOptions' @())){$commandArguments.Add([string]$option)};@('-t',[string]$Duration,'-f',$Route.muxer,'-y',$Output) | ForEach-Object {$commandArguments.Add($_)};@($commandArguments)
}
...
try {
    ...
    Invoke-RequiredScript $retentionValidator @{ArtifactRoot=$artifact};$state.fixtureClosure=Test-FixtureClosure $fixtures
    Invoke-RequiredScript $runtimeValidator @{RuntimeRoot=$runtime;EvidencePath=(Join-Path $stage 'runtime-identity.json')}
    ...
    foreach($row in $state.matrix){
        $route=@($contract.routes|Where-Object id -eq $row.routeId)[0];$extension=if($route.container-eq'mp4'){'mp4'}else{'webm'};$name="$($row.routeId.Split('.')[-1])-$($row.resolutionId)-$($row.threadPolicyId)-$($row.repetitionKind)-$($row.repetitionOrdinal).$extension";$output=Join-Path $media $name
        $record=[ordered]@{row=$row;status='started';startedAtUtc=[DateTimeOffset]::UtcNow.ToString('O');command=$null;measurement=$null;rawEvidence=$null;oracle=$null;error=$null};$state.attempts+=,$record
        try {
            $ffmpegArguments=New-FfmpegArguments $row $route $contract $fixtures $output ([int]$contract.calibration.durationSeconds)
            $record.command=[ordered]@{executable=$ffmpeg;arguments=$ffmpegArguments;components=[ordered]@{inputDemuxers=@($route.inputDemuxers);inputDecoders=@($route.inputDecoders);filters=@($route.filters);streamMaps=@($route.streamMaps);videoEncoder=$route.videoEncoder;audioEncoder=$route.audioEncoder;muxer=$route.muxer;probeDemuxerToken=if($route.container-eq'mp4'){'mp4'}else{'webm'};decodeDemuxerToken=if($route.container-eq'mp4'){'mp4'}else{'webm'};outputDemuxerDescriptor=$route.outputDemuxer;outputDecoders=@($route.outputDecoders);codecThreads=$row.codecThreads;filterThreads=$row.filterThreads;complexFilterThreads='not-applicable-no-complex-filtergraph'}}
            $observed=Invoke-ObservedFfmpeg $ffmpeg $ffmpegArguments (Join-Path $logs $name) $output $false $contract;$record.measurement=$observed.summary
            $record.rawEvidence=[ordered]@{stdout=(Get-FileEvidence "$($logs)\\$name.stdout.txt" $stage);stderr=(Get-FileEvidence "$($logs)\\$name.stderr.txt" $stage);progress=(Get-FileEvidence "$($logs)\\$name.progress.ndjson" $stage);samples=(Get-FileEvidence "$($logs)\\$name.samples.ndjson" $stage)}
            if($observed.exitCode-ne 0){throw "Calibration command failed: $name"};$record.oracle=Invoke-IndependentOracle $ffmpeg $ffprobe $output $row $route $work $fixtures;$record.status='passed'
        } catch {$record.status='failed';$record.error=$_.Exception.ToString()}
        $record.completedAtUtc=[DateTimeOffset]::UtcNow.ToString('O')
    }
    foreach($route in @($contract.routes)){
        $row=[ordered]@{routeId=$route.id;resolutionId='1080p';width=1920;height=1080;threadPolicyId='auto';codecThreads=0;filterThreads=$null;repetitionKind='cancellation';repetitionOrdinal=1};$output=Join-Path $media ("cancellation-$($route.container).partial");$logPrefix=Join-Path $logs "cancellation-$($route.container)"
        $record=[ordered]@{routeId=$route.id;status='started';command=$null;measurement=$null;rawEvidence=$null;cancellation=$null;partialOutputDisposition=$null;error=$null};$state.cancellationProbes+=,$record
        try {
            $ffmpegArguments=New-FfmpegArguments $row $route $contract $fixtures $output ([int]$contract.cancellation.targetOutputDurationSeconds)
            $record.command=[ordered]@{executable=$ffmpeg;arguments=$ffmpegArguments;components=[ordered]@{inputDemuxers=@($route.inputDemuxers);inputDecoders=@($route.inputDecoders);filters=@($route.filters);streamMaps=@($route.streamMaps);videoEncoder=$route.videoEncoder;audioEncoder=$route.audioEncoder;muxer=$route.muxer;codecThreads=0;filterThreads=$null;complexFilterThreads='not-applicable-no-complex-filtergraph'}}
            $probe=Invoke-ObservedFfmpeg $ffmpeg $ffmpegArguments $logPrefix $output $true $contract;$record.measurement=$probe.summary
            $record.rawEvidence=[ordered]@{stdout=(Get-FileEvidence "$logPrefix.stdout.txt" $stage);stderr=(Get-FileEvidence "$logPrefix.stderr.txt" $stage);progress=(Get-FileEvidence "$logPrefix.progress.ndjson" $stage);samples=(Get-FileEvidence "$logPrefix.samples.ndjson" $stage)};$record.cancellation=$probe.cancellation
            $record.partialOutputDisposition=Get-PartialOutputDisposition $output $probe.cancellation.terminationTrigger;if($record.partialOutputDisposition.disposition-eq'retained-after-cleanup-failure'){throw 'Partial-output cleanup failed.'};$record.status=if($probe.cancellation.forcedTermination){'forced-termination-evidence'}else{'not-exercised'}
        } catch {$record.status='failed';$record.error=$_.Exception.ToString()}
    }
    ...
    if($stage -and(Test-Path $stage)){
        ...
        if($AppendRetention -and(Test-Path -LiteralPath $approvedArtifacts -PathType Container)){
            ...
            try {
                ...
                Invoke-RequiredScript $retentionAppender @{ArtifactRoot=$approvedArtifacts;SourceRoot=$stage;SourceTrustBoundary=$trustedProjectParent;GroupId=$group;DestinationName=$destination;Provenance='G0.5 bounded calibration evidence';ProducerRuntimeIdentity=[string[]]@('repository:eng/gate0/manifests/p2-btbn-lgplv3-shared-windows-x64-20260820.json');LicenseRecords=[string[]]@('artifact:p2/runtime/ffmpeg-n8.1.2-44-g7c533d0f86-win64-lgpl-shared-8.1/LICENSE.txt');ProofRunIdentity=[string[]]@("artifact:$destination/g0.5-calibration-evidence.json")}
                Invoke-RequiredScript $retentionValidator @{ArtifactRoot=$approvedArtifacts}
