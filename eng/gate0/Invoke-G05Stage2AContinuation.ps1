...
function Read-G05Stage2AContinuationBootstrapSchedule([string] $Path) {
    ...
    if ($schedule.schemaVersion -ne 2 -or [string]$schedule.scheduleId -ne 'Gate0.G05.Stage2A.ContinuationSchedule.V2' -or [string]$schedule.status -ne 'owner-approved-single-replacement-before-media' -or [string]$schedule.evidenceGroupId -ne 'g05-stage2a-continuation-20260827' -or @($schedule.attempts).Count -ne 72) { throw 'Continuation schedule bootstrap identity is invalid.' }
...
function Read-G05Stage2AContinuationBootstrapAuthorization([string] $Path, [object] $Schedule) {
    ...
    Assert-G05Stage2AContinuationBootstrapProperties $authorization @('schemaVersion','authorizationId','authorizationScope','status','exactCellCount','exactAttemptCount','maximumNewCellCount','scheduleBinding','bindings','continuationProofRunIds','limitations') 'Full continuation authorization'
    ...
    if ($authorization.schemaVersion -ne 2 -or [string]$authorization.authorizationId -ne 'Gate0.G05.Stage2A.ContinuationAuthorization.V2' -or [string]$authorization.authorizationScope -ne 'owner-authorized-stage2a-single-replacement' -or [string]$authorization.status -ne 'owner-authorized-single-replacement-effective' -or [int]$authorization.exactCellCount -ne 12 -or [int]$authorization.exactAttemptCount -ne 72 -or [int]$authorization.maximumNewCellCount -ne 1) { throw 'Full continuation authorization identity is not effective. No media was started.' }
...
function Test-G05Stage2AContinuationSemanticCell{
 ...
 $plan=Resolve-G05Stage2ACellRetentionPlan @($summaries);$mapped=@();foreach($summary in @($plan.attempts)){$attemptRoot=Join-Path $CellRoot ("attempt-$([int]$summary.globalOrdinal-[int]$Rows[0].globalOrdinal+1)");$path=Join-Path $attemptRoot 'summary.json';$binding=New-G05Stage2AAttemptBinding ($Rows|Where-Object globalOrdinal -eq $summary.globalOrdinal) $summary $path $CellRoot $summary.retentionClass $summary.completeClosureReference;if($summary.retentionClass-eq'compact'){Assert-G05Stage2ACompactBinding $binding $CellRoot ([int64]$RetentionContract.Contract.compactPassingRepeatMaximumBytes);Get-ChildItem -LiteralPath $attemptRoot -Force|Where-Object Name -ne 'summary.json'|Remove-Item -Recurse -Force};$row=@($Rows|Where-Object{[int]$_.globalOrdinal-eq[int]$binding.ordinal});$reference=if([string]::IsNullOrWhiteSpace([string]$binding.completeClosureReference)){$null}else{[string]$binding.completeClosureReference};$mapped+=,[ordered]@{attemptId="stage2a-continuation-$($row[0].globalOrdinal)";originalAttemptId="stage2a-$($row[0].originalScheduleOrdinal)";phase=[string]$row[0].phase;ordinal=[int]$row[0].globalOrdinal;retentionClass=[string]$binding.retentionClass;recordPath="future/stage2/v2/$($row[0].proofRunId)/$($binding.recordPath)";recordSha256=[string]$binding.recordSha256;disposition=[string]$binding.disposition;completeClosureReference=$reference}}
...
$contract=[ordered]@{schemaVersion=2;runnerId='Gate0.G05.Stage2A.ContinuationRunner.V2';status=if($ExecuteMedia){'live-execution-requested'}else{'contract-only'};noMediaExecuted=-not[bool]$ExecuteMedia;schedule=[ordered]@{path='eng/gate0/g0.5-stage2a-continuation-schedule.json';sha256=$schedule.Sha256;attemptCount=72;cellCount=12;evidenceGroupId='g05-stage2a-continuation-20260827'};execution=[ordered]@{concurrency=1;maximumNewCellCount=1;freshWarmupsAlwaysExecute=$true;measuredValuesPerCell=5;warmupExcludedFromStatistics=$true;deterministicRouteSuspension=$true;stopAfterApprovedReplacementCell=$true};oracle=[ordered]@{baseline='V3 only';typical='V3 plus exact V4 overlay only';stress='V3 plus exact V5 overlay only';v5FreezePath='eng/gate0/g0.5-lossy-audio-oracle-amendment-v5-freeze.json';reevaluationSummaryPath='eng/gate0/g0.5-stage2a-v5-retained-output-reevaluation-result-summary.json'};retention=[ordered]@{writer='Add-Gate0EvidenceV2Shard.ps1';oneShardPerCell=$true;localAndRemoteValidationAfterEachAppend=$true;incrementalCombinedV1V2Headroom=$true;approvedSourceRoot='StagingRoot'};limitations=@('Proof-only P2 runtime-route single replacement. Remaining continuation cells require another owner approval. No Stage 2B, concurrency comparison, long-form, product, WPF, shipping-runtime, distribution, or legal claim.')}
...
$suspendedRoutes=Restore-G05Stage2AContinuationSuspendedRoutes $schedule.Schedule $ArtifactRoot
 $pendingCells=@($cells|Select-Object -Skip $completed -First ([int]$authorization.Authorization.maximumNewCellCount))
