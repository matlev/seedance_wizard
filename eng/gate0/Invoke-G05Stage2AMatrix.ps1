...
function Invoke-G05Stage2ALiveMatrix([string] $RuntimeRoot, [string] $ArtifactRoot, [string] $StagingRoot, [object] $Schedule, [object] $WorkloadContract, [object] $RetentionContract, [object] $AuthorizationRecord) {
    ...
    foreach ($cellId in @($Schedule.Schedule.attempts | Select-Object -ExpandProperty cellId -Unique)) {
        ...
        $proofRunId="$runId-$($cellId -replace '[^A-Za-z0-9._-]','-')";$destinationName="future/stage2/$proofRunId"
        ...
        $retainedBindings=ConvertTo-G05Stage2ARetainedAttemptBindings @($bindings) $destinationName
        $bindingsPath=Join-Path $cellRoot 'attempt-bindings.json';Write-G05Stage2ASemanticJson $bindingsPath @($retainedBindings)
