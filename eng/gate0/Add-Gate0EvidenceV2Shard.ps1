...
if ([string]::IsNullOrWhiteSpace($Provenance) -or @($ContractIdentity).Count -eq 0 -or @($ProducerRuntimeIdentity).Count -eq 0) { throw 'V2 append metadata is required.' }
 $attempts=@();$liveScheduledRows=$null;$authorizedContinuationCellLimit=12
 if($EvidenceBoundary -eq 'p2-runtime-route') {
   ...
   if($SkipRemoteForIsolatedTest) {
     ...
     $replacementProofId='g05-stage2a-continuation-r1-20260827-stress-720p-webm-eight'
     if($auth.schemaVersion -ne 1 -or $auth.authorizationId -ne 'Gate0.Stage2Evidence.V2.ContinuationAuthorization.V1' -or $auth.authorizationScope -ne 'owner-authorized-v2-continuation' -or $writerProofIds.Count -ne 1 -or $writerProofIds[0] -ne $replacementProofId -or $ProofRunId -ne $replacementProofId -or [string]$fullAuthorization.Schedule.Schedule.attempts[0].proofRunId -ne $replacementProofId -or [int]$fullAuthorization.Authorization.maximumNewCellCount -ne 1){throw 'V2 continuation writer authorization is not the exact approved single replacement proof for this proof run.'}
     $authorizedContinuationCellLimit=[int]$fullAuthorization.Authorization.maximumNewCellCount
...
try {
 ...
 $kind=if($EvidenceBoundary -eq 'containment-no-media'){'infrastructure'}else{'stage2a-continuation-cell'};$limit=if($kind -eq'infrastructure'){2}else{$authorizedContinuationCellLimit}
 ...
 $preflightShardBytes=ConvertTo-Gate0EvidenceV2CanonicalShardUtf8 $preflightShard;$preflightBytes=[int64]$preflightShardBytes.Length;$preflightLines=([Text.UTF8Encoding]::new($false).GetString($preflightShardBytes) -split "`n").Count-1
 ...
 Assert-Gate0EvidenceV2NoReparsePointAncestors $stage $repo;$tmpShard="$shardPath.tmp-$([guid]::NewGuid().ToString('N'))";[IO.File]::WriteAllBytes($tmpShard,(ConvertTo-Gate0EvidenceV2CanonicalShardUtf8 $shard));$s=Read-Gate0EvidenceV2Shard $tmpShard;$last=if(@($root.Index.runs).Count){@($root.Index.runs)[-1]}else{$null}
