...
`Invoke-MediaSmokeTests.ps1` is an explicit local engineering command. Its planned smoke families use generated temporary fixtures, strict inspection/decode, and the explicitly pinned font files under `fonts/`; it never downloads, installs, calls R2, or references product assemblies. Delete temporary output by default; use `-KeepArtifacts` only when investigating a result.
